    ...
    if flag:
        try:
            ...
            channel.basic_publish(
                exchange="", routing_key="error_queue", body=json.dumps(error_message)
            )
            ...
            write_in_error_log(
                f"Error while sending error log to RabbitMQ: {e}", error_log_file
            )
            return error_msg
    else:
        write_in_error_log(error_msg, error_log_file)
...
def read_log_file():
    try:
        ...
        print(f"Error while reading log file: {e}")
        return {
            "user_id": get_uid(),
            "host_name": host_name(),
            "date": today,
...
def delete_old_logs():
    ...
    for file in files_to_delete:
        ...
        if (
            file == f"log_{today}.json"
            or file == f"error_log_{today}.json"
            or file
            == f'log_{(datetime.now() - timedelta(1)).strftime("%Y-%m-%d")}.json'
            or file
            == f'error_log_{(datetime.now() - timedelta(1)).strftime("%Y-%m-%d")}.json'
        ):
            continue
...
def upload_ss_to_server(file_name, api_url):
    error_log_file = os.path.join(
        directory, "logs", f'error_log_{datetime.now().strftime("%Y-%m-%d")}.json'
    )
    try:
