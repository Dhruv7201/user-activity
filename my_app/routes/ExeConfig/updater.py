...
def check_for_updates(current_version: str = Path(..., title="Current Version")):
    if current_version == "0.0.1":
        return JSONResponse(content={"update_required": False, "latest_version": "0.0.1"})
    else:
        return JSONResponse(content={"update_required": True, "latest_version": "0.0.1"})
...
def update_exe():
    ...
    response = FileResponse(new_exe_path, media_type="application/octet-stream", filename=new_exe_path)
...
def download_updater_exe():
    ...
    response = FileResponse("updater_exe.exe", media_type="application/octet-stream", filename="updater_exe.exe")
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"
    return response