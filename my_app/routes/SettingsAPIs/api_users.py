    ...
    async with session.create_client(
        ...
        for date in date_list:
            async for result in paginator.paginate(
                Bucket=MINIO_BUCKET, Prefix=f"ss/{date}/{user_id}"
            ):
                    ...
                    for obj in result["Contents"]:
                        ...
                        if object_name.split("/")[2] == user_id:
                            ...
                            await client.delete_object(
                                Bucket=MINIO_BUCKET, Key=object_name
                            )
                        else:
...
async def delete_user(user_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        remaining_users = await users_collection.find({}, {"user_id": 1})
...
async def put_user(
    ...
    teamname: str = Query(..., min_length=1, max_length=20),
    user_name: str = Query(..., min_length=1, max_length=20),
):
    db = db_connection()
    collection = db["users"]
    if user_name:
...
async def add_monitoring_user(data: dict):
    ...
    collection.insert_one(
        {"username": username, "password": password, "teamname": selectedTeams}
    )
    return JSONResponse(
        content={"username": username, "password": password, "teamname": selectedTeams}
    )
