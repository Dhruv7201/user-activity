...
async def appBar(
        ...
        for app in user["list_of_app"]:
            ...
            for group in group_list:
                if group in window_title:
                    top_seven_group[group] += timedelta(
    ...
    top_seven_group = dict(
        sorted(top_seven_group.items(), key=lambda item: item[1], reverse=True)[:7]
    )
    ...
    result = {"top_seven_group": top_seven_group}
