...
async def dailyreport(request: Request, date: str = Query(None, description="Date (YYYY-MM-DD)", regex=r'\d{4}-\d{2}-\d{2}', example="2023-01-01"),
                        teamname: str = Header(...)):
    global today
    db = db_connection()
    ...
    for doc in today_user_data:
        ...
        working_time_seconds = sum(int(app['used_time'].split(':')[0]) * 3600 +
                                   int(app['used_time'].split(':')[1]) * 60 +
                                   int(app['used_time'].split(':')[2]) for app in doc['list_of_app'])
        working_time = str(timedelta(seconds=working_time_seconds))

        productive_time_seconds = sum(int(app['used_time'].split(':')[0]) * 3600 +
                                      int(app['used_time'].split(':')[1]) * 60 +
                                      int(app['used_time'].split(':')[2]) for app in doc['list_of_app'] if app['window_title'] in productive_apps)
        productive_time = str(timedelta(seconds=productive_time_seconds))
        
        idle_time_seconds = int(idle_time.split(':')[0]) * 3600 + int(idle_time.split(':')[1]) * 60 + int(idle_time.split(':')[2])
