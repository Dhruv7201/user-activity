...
async def attendancePercentage(from_date: str = Query(None, description="Starting date (YYYY-MM-DD)", regex=r'\d{4}-\d{2}-\d{2}', example="2023-01-01"), 
    ...
    for date in all_dates:
        ...
        if date not in user_data_collection.distinct('date'):
            ...
            user_data_for_date = user_data_collection.find({'date': date, 'user_id': {'$in': user_list}})
    ...
    for date in all_dates:
        attendancePercentage['total'] += date_attendance['data'][date]['present'] + date_attendance['data'][date]['absent']
    ...
    attendancePercentage['present'] = round(attendancePercentage['present'] / attendancePercentage['total'] * 100, 2)
    attendancePercentage['absent'] = round(attendancePercentage['absent'] / attendancePercentage['total'] * 100, 2)
