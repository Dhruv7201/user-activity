...
async def total_used_time(request: Request, from_date: str = Query(None, description="Starting date (YYYY-MM-DD)", regex=r'\d{4}-\d{2}-\d{2}', example="2023-01-01"), 
    ...
    for date in all_dates:
        if date in data_collection.distinct('date'):
            ...
            if teamname == 'admin':
                ...
                user_data_for_date = data_collection.find({'date': date, 'user_id': {'$in': list(user['user_id'] for user in user_list_collection.find({'teamname': teamname}))}})
            present_user_list.append(len(set(user['user_id'] for user in user_data_for_date)))
        else:
            present_user_list.append(0)
        if teamname == 'admin':
            absent_user_list.append(len(user_list_collection.distinct('user_id')) - present_user_list[-1])
        else:
            absent_user_list.append(len(user_list_collection.distinct('user_id', {'teamname': teamname})) - present_user_list[-1])
    return JSONResponse(content={'present': present_user_list, 'absent': absent_user_list})
...
async def total_used_time(request: Request, from_date: str = Query(None, description="Starting date (YYYY-MM-DD)", regex=r'\d{4}-\d{2}-\d{2}', example="2023-01-01"),
    ...
    for date in all_dates:
        ...
        if date not in user_data_collection.distinct('date'):
            ...
            # Filter user data based on users present in user_list
            user_data_for_date = user_data_collection.find({'date': date, 'user_id': {'$in': list(user_list)}})
            date_attendance['data'][date]['present'] = len(set(user['user_id'] for user in user_data_for_date))
            date_attendance['data'][date]['absent'] = len(user_list) - date_attendance['data'][date]['present']
    # total users
    total = len(user_list)
