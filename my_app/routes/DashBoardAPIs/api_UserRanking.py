...
async def mostActiveUsers(
    ...
    sorted_user_rankings = dict(
        sorted(user_rankings.items(), key=lambda item: item[1], reverse=True)
    )
    user_rankings_formatted = {
        user: format_time(total_seconds)
        for user, total_seconds in sorted_user_rankings.items()
    }
