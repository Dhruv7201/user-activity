...
async def create_upload_file(request: Request, file: UploadFile = File(...), file_name: str = Path(..., title="The name of the file", description="The name of the file")):
    ...
    with Image.open(os.path.join(file_path, time_file)) as image:
        ...
        client.put_object(bucket_name, location.replace(time_file, thumbnail_name), thumbnail_buffer, len(thumbnail_buffer.getvalue()), content_type='image/png')
