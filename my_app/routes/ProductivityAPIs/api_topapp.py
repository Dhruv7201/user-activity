...
async def topapp(
    ...
    if from_date and to_date:
        ...
        user_data = user_data_collection.find(
            {"date": datetime.now().strftime("%Y-%m-%d"), "user_id": {"$in": user_list}}
        )
    ...
    for user in user_data:
        for app in user["list_of_app"]:
            ...
            top_app[window_title] = top_app.get(window_title, timedelta()) + timedelta(
                hours=used_time.hour, minutes=used_time.minute, seconds=used_time.second
            )
    ...
    result = {"productivity": str(top_app_serializable[0])}
...
async def topTab(
    ...
    for user in user_data:
        for app in user["list_of_app"]:
            ...
            if chrome_tab in window_title:
                top_tab[window_title] = top_tab.get(
                    window_title, timedelta()
                ) + timedelta(
                    hours=used_time.hour,
                    minutes=used_time.minute,
                    seconds=used_time.second,
                )
    ...
    result = {"productivity": str(top_tab_serializable[0])}

    return JSONResponse(status_code=200, content=result)


@api_topapp.get("/topCategory", tags=["ProductivityPage"])
async def topCategory(
    from_date: str = Query(
        None,
        description="Starting date (YYYY-MM-DD)",
        regex=r"\d{4}-\d{2}-\d{2}",
        ...
        for app in user["list_of_app"]:
            ...
            for i, app in enumerate(productivity_list):
                if app in window_title:
                    ...
                    top_category[productive_app] = top_category.get(
                        productive_app, timedelta()
                    ) + timedelta(
                        hours=used_time.hour,
                        minutes=used_time.minute,
                        seconds=used_time.second,
    ...
    for title, time in top_category.items():
        combined_top_category[title] = (
            combined_top_category.get(title, timedelta()) + time
        )

    # Find the app with the highest usage time
    top_category_serializable = max(combined_top_category.items(), key=lambda x: x[1])

    result = {"productivity": str(top_category_serializable[0])}

    return JSONResponse(status_code=200, content=result)


@api_topapp.get("/topUnproductive", tags=["ProductivityPage"])
async def topUnproductive(
    from_date: str = Query(
        None,
        description="Starting date (YYYY-MM-DD)",
        regex=r"\d{4}-\d{2}-\d{2}",
        ...
        for app in user["list_of_app"]:
            ...
            for i, app in enumerate(unproductive_list):
                if app in window_title:
                    ...
                    top_unproductive[unproductive_app] = top_unproductive.get(
                        unproductive_app, timedelta()
                    ) + timedelta(
                        hours=used_time.hour,
                        minutes=used_time.minute,
                        seconds=used_time.second,
    ...
    for title, time in top_unproductive.items():
        combined_top_unproductive[title] = (
            combined_top_unproductive.get(title, timedelta()) + time
        )
    if not combined_top_unproductive:
        result = {"productivity": "No unproductive app found"}
        return JSONResponse(status_code=200, content=result)
    # Find the app with the highest usage time
    top_unproductive_serializable = max(
        combined_top_unproductive.items(), key=lambda x: x[1]
    )

    result = {"productivity": str(top_unproductive_serializable[0])}
