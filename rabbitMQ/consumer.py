...
def process_json_data(data, user_db_data, user_list_collection):
    data['list_of_app'] = [app for app in data['list_of_app'] if
                           app['used_time'] not in ('00:00:00', '00:00:01', '00:00:02')]

    same_user_date = user_db_data.find_one({'user_id': data['user_id'], 'date': data['date']})
    ...
    if data['user_id'] not in list_of_db_user:
        user_list_collection.insert_one({'user_id': data['user_id'], 'teamname': 'admin'})
    if same_user_date:
        for app in data['list_of_app']:
            window_title = app['window_title']
            existing_app = user_db_data.find_one(
                {'user_id': data['user_id'], 'date': data['date'], 'list_of_app.window_title': window_title})
            if existing_app:
                user_db_data.update_one(
                    {'user_id': data['user_id'], 'date': data['date'], 'list_of_app.window_title': window_title},
                    {'$set': {'list_of_app.$.used_time': app['used_time']}})
            else:
                user_db_data.update_one({'user_id': data['user_id'], 'date': data['date']},
